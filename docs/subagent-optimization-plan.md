...
- 状态：已应用到 `v0.2.0` 主规则；`v0.2.2` 根据运营工作台实战补充批量确认、产物可用性状态、mock gate 和预发证据边界。
- 日期：2026-07-11
...
Frontend Agent：
- 任务集总览、批次工作台、明细表格、单项抽屉。
- 本地 mock 和 Playwright 截图验证。

Backend Agent：
- 任务 summary API、分页、状态统计、发布状态、对象存储状态透传。
...
Integration Agent：
- 客户端 -> 主服务/BFF -> AI/Worker -> 对象存储 -> 数据库的联调证据。
```

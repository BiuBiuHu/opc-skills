...
4. UI Agent
   ...
   - 设计 B 类页面前，必须读取 `references/b-side-ui-guidance.md`，并先用 `find-skills` 或开源调研确认是否有可借鉴的后台表格、筛选、批量操作、详情抽屉、虚拟列表、图片预览或审计流程模式；调研结论必须转化为本页面的采纳/拒绝决策。
   ...
   - 需要时复用 `ui-ux-pro-max` 做设计系统与交互质量补充，复用 `ant-design` 做后台表格/表单/分页/ProComponents 选型，复用 `ui-audit` 做发布前 UI 审计。
